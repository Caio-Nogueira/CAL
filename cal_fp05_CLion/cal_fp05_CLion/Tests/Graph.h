...
void Graph<T>::unweightedShortestPath(const T &orig) {
    ...
    while(!process.empty()){
        Vertex<T> *front = process.front();
        process.pop();

        for (Edge<T> edge: front->adj){
            Vertex<T> *dest = edge.dest;
            if (dest->dist == INT_MAX){
                process.push(dest);
                dest->dist = front->dist+1;
                dest->path = front;
            }
        }
    }
...
void Graph<T>::dijkstraShortestPath(const T &origin) {
	...
	orig->dist = 0;
	MutablePriorityQueue<Vertex<T> > process;
	process.insert(orig);
	while (!process.empty()){
	    Vertex<T>* current = process.extractMin();
	    for (Edge<T> edge: current->adj){
	        if (edge.dest->dist > current->dist + edge.weight) {
	            ...
	            if (edge.dest->queueIndex == 0)
	                process.insert(edge.dest);
	            else
	                process.decreaseKey(edge.dest);
	        }
	    }
	}
}
...
void Graph<T>::bellmanFordShortestPath(const T &orig) {
    ...
    for (Vertex<T> *v: vertexSet) {
        for (Edge<T> edge: v->adj) {
            if (v->dist + edge.weight < edge.dest->dist) return; // there are cycles with negative weight
        }
    }
