...
public:
	...
	bool operator ==(const Vertex<T>& g1);
};
...
template <class T>
bool Vertex<T>::operator==(const Vertex<T> &g1) {
    return this->info == g1.info;
}
...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
	...
	if (findVertex(sourc) == NULL || findVertex(dest) == NULL)
	    return false;
	Edge<T>* ed = new Edge<T>(findVertex(dest), w);
	findVertex(sourc)->adj.push_back(*ed);
	return true;
}
...
void Vertex<T>::addEdge(Vertex<T> *d, double w) {
	// TODO (1 line)
	Edge<T>* ed = new Edge<T>(d,w);
	this->adj.push_back(*ed);
}
...
bool Graph<T>::removeEdge(const T &sourc, const T &dest) {
    ...
    return (source != NULL && destination != NULL && source->removeEdgeTo(destination));
}
...
void Graph<T>::dfsVisit(Vertex<T> *v, vector<T> & res) const {
	...
	for (auto it: v->adj){
	    if (it.dest->visited == false) dfsVisit(it.dest, res);
	}
...
vector<T> Graph<T>::bfs(const T & source) const {
	...
	while(!process.empty()){
	    Vertex<T>* current = process.front();
	    process.pop();
	    res.push_back(current->info);
	    for (auto edge: current->adj){
	        Vertex<T> *curr_adj = edge.dest;
	        if (curr_adj->visited == false){
	            process.push(curr_adj);
	            curr_adj->visited = true;
	        }
	    }
	}
...
vector<T> Graph<T>::topsort() const {
	...
	queue<Vertex<T> *> process;
	for (auto vert: vertexSet){
	    if (vert->indegree == 0) process.push(vert);
	}

	while (!process.empty()){
	    Vertex<T> *curr_vert = process.front();
	    process.pop();
	    res.push_back(curr_vert->info);

	    for (auto edge: curr_vert->adj){
	        edge.dest->indegree--;
	        if (edge.dest->indegree == 0) process.push(edge.dest);
	    }
	}
